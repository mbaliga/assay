...
| `runs/<run-id>/findings.sarif` | `.assay/runs/<run-id>/findings.sarif`, referenced via `findingFiles[]` (path/sha256/count) | unchanged encoding (`Sarif.encode`); now summarized, not repeated, in the index |
